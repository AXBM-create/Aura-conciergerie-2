...
export function AuthPanel({ initialMode, onAuthenticated }: AuthPanelProps) {
  ...
  return (
    <section className="relative z-10 w-full max-w-md border border-white/15 bg-[#0b0b0e] p-6 shadow-2xl sm:p-8">
      <div className="mb-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={isSubmitting} className="flex min-h-12 w-full items-center justify-center gap-2 bg-primary px-4 py-3.5 text-xs font-black uppercase tracking-widest text-white transition hover:bg-orange-600 disabled:cursor-wait disabled:opacity-60">
      ...
      <button type="button" onClick={handleGoogle} disabled={isSubmitting} className="min-h-12 w-full border border-white/15 px-4 py-3 text-xs font-bold uppercase tracking-widest text-white transition hover:border-white/35 disabled:opacity-50">
        Continuer avec Google
      </button>

      <p className="mt-6 text-center text-xs text-white/45">
        {isSignup ? 'Vous avez déjà un accès ?' : 'Pas encore membre ?'}{' '}
        <button type="button" onClick={() => switchMode(isSignup ? 'login' : 'signup')} className="font-bold text-primary hover:text-accent">
